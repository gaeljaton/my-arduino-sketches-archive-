...
//########################### VARIABLES TEMPS ##################################
int duree=1; // durée totale en min
...
void loop () {
  ...
  ++midi; 
  if(midi>23){midi=0;}
  if(midi<12){analogWrite(6,midi*22+1);digitalWrite(2,HIGH);digitalWrite(3,LOW);}else{analogWrite(6,(23-midi)*22+1);digitalWrite(2,LOW);digitalWrite(3,HIGH);}
  if (midi==0){digitalWrite(13,HIGH); }else{digitalWrite(13,LOW);lcd.setCursor(0, 0); lcd.print(T);
