...
void Nunchuk::begin()
{
  ...
  Wire2.beginTransmission(NUNCHUK_TWI_DEVICE_ADDRESS);	// device address
  Wire2.write(0xF0);		        // 1st initialisation register
  Wire2.write(0x55);		        // 1st initialisation value
  Wire2.endTransmission();
  delay(1);
  
  Wire2.beginTransmission(NUNCHUK_TWI_DEVICE_ADDRESS);
  Wire2.write(0xFB);		        // 2nd initialisation register
  Wire2.write(0x00);		        // 2nd initialisation value
  Wire2.endTransmission();
  delay(1);
    
  // write the crypto key (zeros), in 3 blocks of 6, 6 & 4.
  Wire2.beginTransmission(NUNCHUK_TWI_DEVICE_ADDRESS);
  Wire2.write(0xF0);		        // crypto key command register
  Wire2.write(0xAA);		        // writes crypto enable notice
  Wire2.endTransmission();
  delay(1);
  
  Wire2.beginTransmission(NUNCHUK_TWI_DEVICE_ADDRESS);
  Wire2.write(0x40);		        // crypto key data address
  for (i = 0; i < 6; i++) {
      Wire2.write(0x00);		        // writes 1st key block (zeros)
  }
  Wire2.endTransmission();
  delay(1);
  
  Wire2.beginTransmission(NUNCHUK_TWI_DEVICE_ADDRESS);
  Wire2.write(0x40);		        // writes memory address
  for (i = 6; i < 12; i++) {
  ...
  Wire2.write(0x40);		        // writes memory address
  for (i = 12; i < 16; i++) {
      Wire2.write(0x00);		        // writes 3rd key block (zeros)
  }
  Wire2.endTransmission();
  delay(1);
...
void Nunchuk::read()
{
  ...
  Wire2.requestFrom( NUNCHUK_TWI_DEVICE_ADDRESS, NUNCHUK_TWI_BUFFER_SIZE );
  delayMicroseconds( NUNCHUK_TWI_DELAY_MICROSEC );
  
  // Read the new data
  if( NUNCHUK_TWI_BUFFER_SIZE==Wire2.available() )
  {
    _ok = 1;
    for( i = 0; i<NUNCHUK_TWI_BUFFER_SIZE; i++ )
    {
      _buf[i] = _decode_byte( (uint8_t)Wire2.read() );
    }
  }
  else
  {
  ...
  Wire2.beginTransmission( NUNCHUK_TWI_DEVICE_ADDRESS );
  Wire2.write( NUNCHUK_TWI_CMD_ZERO );
